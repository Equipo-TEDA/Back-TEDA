...
router_3 = APIRouter(prefix="/pagina_2",responses={404:{"message":"No encontrado"}})
...
@router_3.get("/search_efficiency")
async def search_efficiency(
    ...
    try:
        base_query_won = """
            ...
            WHERE year(s.modification_date) = YEAR(curdate()) - 1 
...
#-----------------------------------------------------------------------------------------------
#Cantidad de búsquedas totales (Página 2)

@router_3.get("/count_total_searches")
async def count_total_searches(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
...
#--------------------------------------------------------------------------
#Cantidad de vacantes totales en el año anterior

@router_3.get("/vacancies_past_year")
async def vacancies_past_year(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        base_query = """
            ...
            INNER JOIN client AS c ON s.client_id = c.id
            INNER JOIN status_search AS ss ON s.status_search_id = ss.id
            WHERE year(s.modification_date) = YEAR(curdate()) - 1  AND s.id <> 22
        """
...
#-----------------------------------------------------------------------
#Cantidad de busquedas ganadas en el año anterior

@router_3.get("/earned_searchs_past_year")
async def earned_searchs_past_year(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        base_query = """
            ...
            INNER JOIN client AS c ON s.client_id = c.id
            INNER JOIN status_search AS ss ON s.status_search_id = ss.id
            WHERE year(s.modification_date) = YEAR(curdate()) - 1  AND s.status_search_id = 3 AND s.id <> 22
        """
...
#---------------------------------------------------------------------------
#Cantidad de busquedas cerradas en el año anterior

@router_3.get("/closed_searchs_past_year")
async def closed_searchs_past_year(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        base_query = """
            ...
            INNER JOIN client AS c ON s.client_id = c.id
            INNER JOIN status_search AS ss ON s.status_search_id = ss.id
            WHERE year(s.modification_date) = YEAR(curdate()) - 1  AND s.status_search_id = 2 AND s.id <> 22
        """
...
#-----------------------------------------------------------------------
# Cantidad de búsquedas TRABAJANDO (tarjeta)

@router_3.get("/working_searchs")
async def working_searchs(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        base_query = """
            ...
            INNER JOIN client AS c ON s.client_id = c.id
            INNER JOIN status_search AS ss ON s.status_search_id = ss.id
            WHERE year(s.modification_date) = YEAR(curdate()) - 1  AND s.status_search_id IN (1, 5, 4) AND s.id <> 22
        """
...
#-----------------------------------------------------------------------
# Cantidad de búsquedas ABIERTAS (tarjeta)

@router_3.get("/open_searchs")
async def open_searchs(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        base_query = """
            ...
            INNER JOIN client AS c ON s.client_id = c.id
            INNER JOIN status_search AS ss ON s.status_search_id = ss.id
            WHERE year(s.modification_date) = YEAR(curdate()) - 1  AND s.status_search_id = 1 AND s.id <> 22
        """
...
#-----------------------------------------------------------------------
# Cantidad de búsquedas Stand-By(tarjeta)

@router_3.get("/standby_searchs")
async def standby_searchs(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        base_query = """
            ...
            INNER JOIN client AS c ON s.client_id = c.id
            INNER JOIN status_search AS ss ON s.status_search_id = ss.id
            WHERE year(s.modification_date) = YEAR(curdate()) - 1  AND s.status_search_id = 5 AND s.id <> 22
        """
...
#-----------------------------------------------------------------------
# Cantidad de búsquedas hibernando (tarjeta)

@router_3.get("/hibernating_searchs")
async def hibernating_searchs(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        base_query = """
            ...
            INNER JOIN client AS c ON s.client_id = c.id
            INNER JOIN status_search AS ss ON s.status_search_id = ss.id
            WHERE year(s.modification_date) = YEAR(curdate()) - 1  AND s.status_search_id = 4 AND s.id <> 22
        """
...
#-----------------------------------------------------------------------
# Cantidad de vacantes, en búsquedas GANADAS (tarjeta)

@router_3.get("/earned_search_vacancies")
async def earned_search_vacancies(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
...
#-----------------------------------------------------------------------
# Cantidad de vacantes, en búsquedas CERRADAS (tarjeta)

@router_3.get("/closed_search_vacancies")
async def closed_search_vacancies(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
...
@router_3.get("/earned_searchs_per_month")
async def earned_searchs_per_month(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
    ...
    try:
        query_str = """
            ...
            WHERE YEAR(s.modification_date) = (YEAR(curdate()) - 1) AND s.status_search_id = 3
        """
    ...
    #-----------------------------------------------------------------------
# Tabla 
@router_3.get("/table_client_status_search")
async def tabla(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
...
@router_3.get("/working_search_vacancies")
async def working_search_vacancies(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
...
@router_3.get("/stand_search_vacancies")
async def stand_search_vacancies(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
...
@router_3.get("/hibernating_search_vacancies")
async def hibernating_search_vacancies(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
...
@router_3.get("/open_search_vacancies")
async def open_search_vacancies(
    client_name: Optional[str] = Query(None), 
    search_name: Optional[str] = Query(None),
    status_name: Optional[str] = Query(None),
    db: Session = Depends(get_db)
